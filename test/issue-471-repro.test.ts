...
function planMode(pi: Parameters<typeof planModeExtension>[0], options?: Parameters<typeof planModeExtension>[1]) {
...
test("starting a new Plan-mode workflow supersedes the active implementation", async () => {
  ...
  assert.deepEqual(transformed.messages.slice(1), [{ role: "user", content: "design a replacement" }]);
});
